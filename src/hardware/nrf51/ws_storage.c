...
#define STORAGE_RECORDS_BUFFER_LENGTH    4

#define STORAGE_FDS_RECORD_INIT(p_chunk)                                         { STORAGE_FILE_ID, 0x0000, { (p_chunk), 0x0001 } }
#define WS_STORAGE_FDS_RECORD_CHUNK_INIT(p_data, size_in_words)                     { p_data, size_in_words }
#define STORAGE_DATA_WRAPPER_INIT(p_wrapper)                                     ((WS_StorageDataWrapper_t) { { 0x00 }, STORAGE_FDS_RECORD_INIT(&(p_wrapper)->chunk), WS_STORAGE_FDS_RECORD_CHUNK_INIT(&(p_wrapper)->data, 0x0000), false })
#define WS_STORAGE_FDS_RECORD_SET_DATA(p_wrapper_m, key_m, p_data_m, size_m)        do{ memcpy(&(p_wrapper_m)->data, p_data_m, MIN(size_m, STORAGE_DATA_BUF_SIZE)); (p_wrapper_m)->chunk.p_data = (p_wrapper_m)->data; (p_wrapper_m)->chunk.length_words = MIN(size_m, STORAGE_DATA_BUF_SIZE) / STORAGE_WORD_SIZE; if (MIN(size_m, STORAGE_DATA_BUF_SIZE) % STORAGE_WORD_SIZE) { (p_wrapper_m)->chunk.length_words++; } (p_wrapper_m)->record.key = key_m; }while(0)
...
static WS_StorageDataWrapper_t ws_recordsBuffer[STORAGE_RECORDS_BUFFER_LENGTH];

winsens_status_t WS_StorageInit(void)
{
    uint_fast8_t i = 0;
    ...
    {
        ws_recordsBuffer[i] = STORAGE_DATA_WRAPPER_INIT(&ws_recordsBuffer[i]);
    }
...
winsens_status_t WS_StorageRead(
    storage_record_id_t recordId,
    uint32_t size,
    uint8_t* data)
{
    ...
    if (ret == FDS_SUCCESS)
    {
        ...
        memcpy(data, flash_record.p_data, MIN(size, flash_record.p_header->tl.length_words * STORAGE_WORD_SIZE));
...
winsens_status_t WS_StorageWrite(
    storage_record_id_t recordId,
    uint32_t size,
    uint8_t* data)
{
    ...
    {
        ILOG_ERROR("Memory leak while writing to a storage (%u bytes)\r\n", sizeof(fds_record_t) + sizeof(fds_record_chunk_t) + size);
    }
...
//                WS_LOG_ERROR("Initialization failed\r\n");
        ...
        case FDS_EVT_WRITE:
        {
            ...
            for (i = 0; i < STORAGE_RECORDS_BUFFER_LENGTH; ++i)
            {
                if (p_fds_evt->write.record_key == STORAGE_FDS_RECORD_GET_KEY(&ws_recordsBuffer[i]))
                {
                    ws_recordsBuffer[i].used = false;
                }
            }
