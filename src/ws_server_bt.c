...
static void ws_on_enabled_write(WS_Window_e window, bool value)
{
    ...
    WS_LOG_INFO("ws_on_enabled_write %u", value);
    ws_update_subscribers(window, e);
}
    ...
    pm_evt_t const * p_evt)
{
    ...
    switch (p_evt->evt_id)
    {
        ...
        {
            // Assert.
            WS_LOG_NRF_ERROR_CHECK(p_evt->params.peer_data_update_failed.error);
        } break;

        case PM_EVT_PEER_DELETE_FAILED:
        {
            // Assert.
            WS_LOG_NRF_ERROR_CHECK(p_evt->params.peer_delete_failed.error);
        } break;

        case PM_EVT_PEERS_DELETE_FAILED:
        {
            // Assert.
            WS_LOG_NRF_ERROR_CHECK(p_evt->params.peers_delete_failed_evt.error);
        ...
        {
            // Assert.
            WS_LOG_NRF_ERROR_CHECK(p_evt->params.error_unexpected.error);
        } break;
    ...
    void * p_context)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        ...
        case BLE_GAP_EVT_TIMEOUT:
            WS_LOG_DEBUG("BLE_GAP_EVT_TIMEOUT %u.", p_ble_evt->evt.gap_evt.params.timeout.src);
            break;
        ...
        default:
            WS_LOG_DEBUG("ws_on_ble_evt event %u not handled", p_ble_evt->header.evt_id);
...
static void WS_ChangeState(
    const WS_ServerBtState_t *newState)
{
    WS_LOG_DEBUG("%s -> %s", (uint32_t) WS_ConvertState(ws_currentState->stateId), (uint32_t) WS_ConvertState(newState->stateId));
